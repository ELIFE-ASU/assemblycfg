...
def test_virt_obj():
    ...
    The function verifies that the virtual objects produced by the `repair_with_pathways` function:
    ...
    s_inpt = "abracadabra"
    # Get the result
    _, virt_obj, _ = CFG.repair_with_pathways(s_inpt)
...
def test_path():
    ...
    The function verifies that the `repair_with_pathways` function produces a path with the
    ...
    s_inpt = "abracadabra"
    # Get the assembly calculator result
    _, _, path = CFG.repair_with_pathways(s_inpt, f_print=False)
...
def test_batch():
    ...
    for i in range(len(s_inpts)):
        # Get the assembly calculator result
        ai, virt_obj, _ = CFG.repair_with_pathways(s_inpts[i], f_print=False)
...
def test_joint():
    ...
    ai_ref = 8
    # Get the assembly calculator result
    ai, virt_obj, _ = CFG.repair_with_pathways(s_inpt, f_print=True, debug=True)
    # Check if the result is correct
    assert ai == ai_ref
...
def test_joint2():
    ...
    ai_ref = 6
    ai, virt_obj, _ = CFG.repair_with_pathways(s_in, f_print=True, debug=True)
    assert ai == ai_ref
